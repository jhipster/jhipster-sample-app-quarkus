...
public class UserService {
    ...
    @Inject
    public UserService(BCryptPasswordHasher passwordHasher, AuthorityRepository authorityRepository, UserRepository userRepository) {
...
//                this.clearUserCaches(user);
    ...
    public User registerUser(UserDTO userDTO, String password) {
        ...
        authorityRepository.findByIdOptional(AuthoritiesConstants.USER).ifPresent(authorities::add);
...
//        this.clearUserCaches(existingUser);
    ...
    public User createUser(UserDTO userDTO) {
        ...
        if (userDTO.authorities != null) {
            Set<Authority> authorities = userDTO.authorities.stream()
                .map(authority -> authorityRepository.findByIdOptional(authority))
...
//                this.clearUserCaches(user);
    ...
    public Optional<UserDTO> updateUser(UserDTO userDTO) {
        return userRepository.findByIdOptional(userDTO.id).map(user -> {
            user.login = userDTO.login.toLowerCase();
            user.firstName = userDTO.firstName;
            user.lastName = userDTO.lastName;
            if (userDTO.email != null) {
            ...
            userDTO.authorities.stream()
                .map(authorityRepository::findByIdOptional)
                .filter(Optional::isPresent)
                .map(Optional::get)
...
//                this.clearUserCaches(user);
    ...
    public List<UserDTO> getAllManagedUsers() {
        return userRepository.findAllByLoginNot(Page.ofSize(20), Constants.ANONYMOUS_USER)
